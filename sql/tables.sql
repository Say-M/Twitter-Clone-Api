...
CREATE TABLE IF NOT EXISTS "comments" (
	...
	"tweetId" UUID REFERENCES tweets(id) ON DELETE CASCADE,
	"retweetId" UUID REFERENCES tweets(id) ON DELETE CASCADE,
	content VARCHAR(280) NOT NULL,
	"createdAt" TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
	"updatedAt" TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS "likes" (
	...
	"userId" UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
	"tweetId" UUID REFERENCES tweets(id) ON DELETE CASCADE,
	"retweetId" UUID REFERENCES retweets(id) ON DELETE CASCADE,
	"commentId" UUID REFERENCES comments(id) ON DELETE CASCADE
);
